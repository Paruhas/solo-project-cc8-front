...
function Register() {
  ...
  const handlerInputRegisChange = (e) => {
    ...
    setInputRegis((prev) => ({ ...prev, [name]: value }));
  };
  ...
  const handlerSubmitRegis = async (e) => {
    ...
    try {
      if (!inputRegis.email)
        return setErrorRegis((prev) => ({ ...prev, err: "email is required" }));
      if (!inputRegis.password)
        return setErrorRegis((prev) => ({
          ...prev,
          err: "password is required",
        }));
      if (!inputRegis.confirmPassword)
        return setErrorRegis((prev) => ({
          ...prev,
          err: "confirmPassword is required",
        }));
  ...
  return (
    <div className="content-center-loginPage-form-div-outside">
      <form className="content-center-loginPage-form-div">
        <h2 className="loginPage-form-div-header-text">สมัครสมาชิก</h2>
        ...
        <input
          ...
          onChange={handlerInputRegisChange}
        />
        <label
        ...
        <input
          type={changeTextType.confirmPassword}
          id="register-password-confirm"
          className="loginPage-form-div-input"
          name="confirmPassword"
          value={inputRegis.confirmPassword}
          onChange={handlerInputRegisChange}
        />
        ...
        {errorRegis.err && (
          <span className="error-box">
            <h4>{errorRegis.err}</h4>
          </span>
        )}
        <button
          type="submit"
          className="loginPage-form-div-submit-btn"
          onClick={handlerSubmitRegis}
        >
          สมัครสมาชิก
        </button>
      </form>
