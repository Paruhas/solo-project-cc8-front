...
import { setToken , getToken, setRole } from "../../services/localStorageService";
...
function Login() {
  ...
  const handlerChangeTextType = (e) => {
    ...
    setChangeTextType("text") : setChangeTextType("password");
  };
  ...
  return (
    <div className="content-center-loginPage-form-div-outside">
      <form className="content-center-loginPage-form-div">
        ...
        <input type="text" id="login-email" name="email" className="loginPage-form-div-input" value={input.email} onChange={handlerInputChange}/> 
        
        <label htmlFor="login-password" className="loginPage-form-div-label-text">
          <h4>
            Password
          </h4>
        </label>
        <input type={changeTextType} id="login-password" name="password" className="loginPage-form-div-input" value={input.password} onChange={handlerInputChange}/>
        { !(input.password === "") && (   
          (changeTextType === "password") ? 
        ...
        {error.err && <span className="error-box"><h4>{error.err}</h4></span>} 
        <button type="submit" className="loginPage-form-div-submit-btn" onClick={handlerSubmit}>
          เข้าสู่ระบบ
        </button>
      </form>
    </div>
